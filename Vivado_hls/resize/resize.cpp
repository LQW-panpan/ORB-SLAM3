...
#pragma HLS INLINE off
    ...
    //1：640*400 1.2：533*333 1.44：444*278’ 1.729：370*231 2.071：309*193 2.49：257*161’ 2.99：214*134’ 3.575：179*112’
    ...
    //1.44
	if(scale.range(15, 0) == 0b0101110000101000)
	    new_height = new_height+1;
    //2.49
    if(scale.range(15, 0) == 0b1001111101011100)
    	new_height = new_height+1;
    //2.99
	if(scale.range(15, 0) == 0b1011111101011100)
	    	new_height = new_height+1;
	//3.575
	if(scale.range(15, 0) == 0b1110010011001100)
			new_height = new_height+1;
...
void process_scale_1(hls::stream<ap_axiu<INPUT_STREAM_BIT, 1, 1, 1> > &srcStream, hls::stream<ap_axiu<OUTPUT_STREAM_BIT, 1, 1, 1> > &outStream)
{
    ...
    {
//    	640*400/16=16000
#pragma HLS LOOP_TRIPCOUNT min=16000 max=16000 avg=16000
...
#pragma HLS UNROLL
            ...
            write_tmp = 0;
            if (rmn_num > 0)
                write_tmp = data.range((int)(pixel_num * PIXEL_BIT - 1), (int)((pixel_num - rmn_num) * PIXEL_BIT));
        }
        else
        {
            ...
            {
                write_tmp.range((int)(OUTPUT_BIT - 1), (int)(rmn_num * PIXEL_BIT)) = data.range((int)(OUTPUT_BIT - 1 - rmn_num * PIXEL_BIT), 0);
...
#pragma HLS UNROLL
            ...
            else
            {
                if (pixel_num > 0)
                {
                    write_tmp.range((int)((rmn_num + pixel_num) * PIXEL_BIT - 1), (int)(rmn_num * PIXEL_BIT)) = data.range((int)(pixel_num * PIXEL_BIT - 1), 0);
                }
...
#pragma HLS INLINE off
    ...
    //  将输入图像640*400*8，处理成一个个16*8的batch
...
#pragma HLS PIPELINE
            ...
            else
            {
                ...
                if (rmn_num > 0)
                {
                    ...
                    write_tmp.range(write_num - 1, rmn_num) = data.range((int)(write_num - rmn_num - 1), 0);
                    rmn_num = INPUT_BIT - (write_num - rmn_num);
                }
                //	对于第一行最后一个batch有rmn_num>0，因为640/16的余数小于16
                else
                {
                    write_tmp = 0;
...
Row_Loop:
    ...
    {
//    	400-(2-1)=399
#pragma HLS LOOP_TRIPCOUNT min=399 max=399 avg=399
        for (ap_uint<WIDTH_BIT> read_ind = 0; read_ind < unit_num; read_ind++)
        {
#pragma HLS LOOP_TRIPCOUNT min=40 max=40 avg=40
#pragma HLS PIPELINE
...
void process_output(hls::stream<ap_uint<8 + PROCESS_BIT> > &selectData, hls::stream<ap_axiu<OUTPUT_STREAM_BIT, 1, 1, 1> > &outStream)
{
    ...
    {
//    	400/1.2=333
#pragma HLS LOOP_TRIPCOUNT min=333 max=333 avg=333
...
#pragma HLS UNROLL
                ...
                {
                    if (pixel_num > 0)
                        write_tmp.range((rmn_num + pixel_num) * PIXEL_BIT - 1, rmn_num * PIXEL_BIT) = data.range((int)(pixel_num * PIXEL_BIT - 1), 0);
                    rmn_num = rmn_num + pixel_num;
                }
            }
        }
