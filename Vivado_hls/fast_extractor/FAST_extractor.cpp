...
void FAST(hls::stream <ap_axiu<32, 1, 1, 1> > &cfgStream, hls::stream <ap_axiu<INPUT_STREAM_BIT, 1, 1, 1> > &srcStream,
          hls::stream <ap_axiu<32, 1, 1, 1> > &cfgoutStream, hls::stream <ap_axiu<OUTPUT_STREAM_BIT_EVEN, 1, 1, 1> > &outPixelStream,
          hls::stream <ap_axiu<OUTPUT_STREAM_BIT_EVEN, 1, 1, 1> > &outFASTStream) {
...
void process_output(hls::stream <ap_uint<PROCESS_BIT> > &gausData, hls::stream <ap_uint<PROCESS_BIT> > &FASTData,
                    hls::stream <ap_axiu<OUTPUT_STREAM_BIT_EVEN, 1, 1, 1> > &outPixelStream,
                    hls::stream <ap_axiu<OUTPUT_STREAM_BIT_EVEN, 1, 1, 1> > &outFASTStream) {
    ap_axiu<OUTPUT_STREAM_BIT_EVEN, 1, 1, 1> outPixel, outFAST;
    ap_uint<10> cnt_lqw = 0;
    for (ap_uint<HEIGHT_BIT> row_ind = 0; row_ind < height; row_ind++) {
    	cnt_lqw = 0;
    	for (ap_uint<WIDTH_BIT> col_ind = 0; col_ind < unit_num; col_ind++) {
            ap_uint<PROCESS_BIT> pixel_data = gausData.read();
            ap_uint<PROCESS_BIT> FAST_data = FASTData.read();
            for (ap_uint<3> process_ind = 0; process_ind < 4; process_ind++) {
				if (cnt_lqw < width){
            		outPixel.data = pixel_data.range(process_ind*8+7,process_ind*8);
					outFAST.data = FAST_data.range(process_ind*8+7,process_ind*8);

					outPixel.keep = 1;
					outFAST.keep = 1;

					if (row_ind==height-1 && col_ind==unit_num-1 && cnt_lqw==width-1){
						outPixel.last = 1;
						outFAST.last = 1;
					}else{
						outPixel.last = 0;
